...
public class App extends Application {
    ...
    private void firstScreen() {
        ...
        sbmBtn.setOnAction((e) -> {
            // if ((ampField.getText().toString() != null) &&
            // (freqField.getText().toString() != null) && (sizeField
    ...
    private void secondScreen() {
        ...
        // Thread dataThread = new Thread(addToQueue);
        dataThread = new Thread(addToQueue);
        dataThread.start();
        prepareData();
        server = new Server(dataQ.size(), serverData);
        executor.execute(server);

        // Thread newThread = new Thread(new Graph());
        // newThread.start();
    ...
    // if (seriesData > frequency * 3) {
    // dSeries.getData().remove(0);
    // xAxis.setLowerBound(seriesData - 500);
    // xAxis.setUpperBound(seriesData + 1);
    // }
    ...
    // private class Graph implements Runnable {
    // @Override
    // public void run() {
    // prepareData();
    // }
